...
def generate_kite_model_and_orbit(windings, intervals_per_winding, time_per_winding):
    ...
    options['model.system_bounds.u.dcoeff'] =  [np.array([-.025, -1]), np.array([.05, 1])]
    ...
    trial.build()
    trial.optimize(final_homotopy_step = 'power')
...
z = model['rootfinder'](z_0, x_awe, u_awe)
nx = x.shape[0]
nu = u_shape[0]
...
data = {
    ...
    'y_ref': x_ref_array[:, 1],
    'z_ref': x_ref_array[:, 2],
    'x_sim_casados': x_sim_padded[:, 0],
    'y_sim_casados': x_sim_padded[:, 1],
    'z_sim_casados': x_sim_padded[:, 2]
}      
