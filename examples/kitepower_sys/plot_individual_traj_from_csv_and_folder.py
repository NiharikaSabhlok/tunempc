...
DATA_DIR   = Path("F:/Thesis/Parameter_sweep_analysis/loop_t_34_N_68")  # contains the per-run CSVs
...
cmap = plt.cm.viridis
colors = cmap(np.linspace(0.2, 0.95, num_shades))  # avoid too-light colors
...
if plot_type == 'stacked':
    ...
    # save + show
    out_path = OUT_DIR / f"Optimal_trajectories_for_T+{time_iterated}.png"
    ...
    for i, param in enumerate(final_param_list):
        ...
        for entry in grouped_data[param]:
            ...
            *data_columns, _label_text = entry
            ...
            # label only once per parameter group
            label = f"β={beta:g}, a={acc:g}, t={t}" if first_label else None
            ax.plot(x, y, z, lw=1.8, color=colors[2], label=label)
            first_label = False
            ...
            # legend outside
            ax.legend(labels=f"β={beta:g}, a={acc:g}, t={t}", loc='center left',
                    bbox_to_anchor=(1.02, 0.5), fontsize='small', title='Parameters', frameon=True)
            ...
            # save + show
            out_path = OUT_DIR / "all_plots" /f"Optimal_trajectories_for_beta_{beta}_acc_reg_{acc}_T_{t}.png"
            plt.savefig(out_path, dpi=200, bbox_inches='tight')
            print(f"Saved plot to: {out_path}")
            # plt.show()
