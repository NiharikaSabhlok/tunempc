...
CSV_PATH   = Path("F:/Thesis/Parameter_sweep_analysis/Analysis_results/files/filtered_equivalence_types.csv")          # CSV with columns: t, n, beta, accreg (names can vary; we auto-map)
...
user_pickle_folder    = Path("F:/Thesis/Parameter_sweep_analysis/Analysis_results/files/input_files")  # folder with user_input pickles
convex_pickle_folder  = Path("F:/Thesis/Parameter_sweep_analysis/Analysis_results/files/convexified_files")  # folder with convexified reference pickles
ROW_RANGE  = (299, 300)              # inclusive (0-based): process rows 10..24
...
STATE_IDX_TO_PLOT_2  = 2 
OUT_ROOT   = Path("F:/Thesis/Parameter_sweep_analysis/Analysis_results/files")
index_lt = 8
index_dl_t = 9
index_lambda = -1
...
def plot_power_tracking(outdir: Path, log, log_ref, user_input):
    ...
    if "power_ref" in log_ref and len(log_ref["power_ref"]) >= Nsim:
        ...
        plt.step(t, Pref, where="post", label="Reference", linewidth=2)

    # controllers
    for nm in names:
        Pk = np.array([float(v) for v in log["power"][nm]]).ravel()
        plt.step(t, Pk, where="post", label=nm, alpha=0.9)
...
def plot_trajectory_all(outdir: Path, log, log_ref, sol, user_input,
    ...
    ax.plot(Xr, Yr, Zr, label="Reference", linewidth=2)
    ...
    for nm in names:
        ...
        ax.plot(Xc, Yc, Zc, label=nm, alpha=0.9)
...
def plot_trajectory_per_controller(outdir: Path, log, log_ref, sol, user_input,
    ...
    for nm in names:
        ...
        ax  = fig.add_subplot(111, projection='3d')
        ax.plot(Xr, Yr, Zr, label="Reference", linewidth=2)
        ax.plot(Xc, Yc, Zc, label=nm, alpha=0.95)
...
def run_one_combo(T, Nmpc, beta, accreg, seed):
    ...
    with open(outdir/"log.txt", "wb") as f:
        pickle.dump(log, f)
