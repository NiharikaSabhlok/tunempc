...
ROW_RANGE  = (38,39)              # inclusive (0-based): process rows 10..24 #299
...
def closed_loop_with_noise(ctrls, F, user_input, sol, Nsim, gauss_w_std=0.0, gauss_v_std=0.0, seed=0):
    ...
    for name, ctrl in ctrls.items():
        ...
        for k in range(Nsim):
            ...
            # MPC action
            u = ctrl.step(x_meas)
            # ipopt_iteration, ipopt_twall, ipopt_status, u = ctrl.step(x_meas)                          # online control from Pmpc. :contentReference[oaicite:7]{index=7}
            # u=sol_dict['u']
            # ipopt_info = sol_dict['ipopt_stats']
            ...
            # log["ipopt_iterations"][name].append(ipopt_iteration)
            # log["ipopt_twall"][name].append(ipopt_twall)
            # log["ipopt_status"][name].append(ipopt_status)
