...
beta_0 = 0.5
acc_reg=1
filefolder = 'F:\Thesis\Parameter_sweep_analysis\T_28_N_56\\'
file_name = 'Beta_and_acc_reg_wo_warmstarting'
...
trial_default = awe.Trial(options, 'lift')
trial_default.build()
...
if TWO_PARAM_ANALYSIS :        
    ...
    for idy, beta_0 in enumerate([0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5]):
        for idx, acc_reg in enumerate([0,1,1e6,5e6,1e7,5e7,1e8]):        
    ...
    avg_power_df.to_csv("avg_power_and_twall_for_beta_and_Acc_N_56_T_28.txt", index=False, header=True, sep=',',mode='a')
    ...
    for idx, param_val in enumerate([1e6,1e7,1e8, 2e8, 5e8, 1e9,2e9, 2e9, 4e9, 5e9, 7e9,1e10]):
        ...
        if idx == 0:
            ...
            plot_dict = trial_default.visualization.plot_dict
            avg_power = plot_dict['power_and_performance']['avg_power']
            avg_power_array.append([beta_0, param_val, avg_power.full().item()])
        ...
        trial_default.write_to_csv(filename=filefolder + '{}_sol_beta{}_acc_reg_{}_N_{}_time_{}'.format(file_name, beta_0, param_val, N, time_per_winding), frequency=10., rotation_representation='dcm')
        print("Wrote file to the folder")
    acg_power_df = pd.DataFrame(avg_power_array, columns=["beta_0", "acc_reg", "avg_power"])
    # Write to a .txt file as comma-separated values
    acg_power_df.to_csv("avg_power_output_for_acc_reg_higher_multiplier_2.txt", index=False, header=True, sep=',')
    plt.figure()
    plt.plot(acg_power_df["acc_reg"], acg_power_df["avg_power"], marker='o')
    plt.xlabel("acc_reg")
    plt.ylabel("Average Power")
    plt.title("Average Power vs acc_reg")
    plt.grid(True)
    plt.tight_layout()
    plt.show()