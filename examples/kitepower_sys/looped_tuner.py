...
directory_path="F:\Thesis\Parameter_sweep_analysis"
...
def tuner_convexification(pickle_file, beta, acc_reg, T, N):
    ...
    Logger.logger.info(18*' '+'Convexify MPC Hessian...')
    Logger.logger.info(60*'=')
    Logger.logger.info('')
...
def run(root: Path, out_csv: Path):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        folder = Path(dirpath)
        mfold = FOLDER_RE.match(folder.name)
        if not mfold:
            continue
        T_folder = int(mfold.group("T"))
        N_folder = int(mfold.group("N"))
...
if __name__ == "__main__":
    ...
    OUT  = Path("tuner_convexification_results_wo_warmstarting.csv")
    run(ROOT, OUT)
