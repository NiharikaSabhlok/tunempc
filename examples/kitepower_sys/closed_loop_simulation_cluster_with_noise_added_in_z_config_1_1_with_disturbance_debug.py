...
ROW_RANGE  = (0,0)              # inclusive (0-based): process rows 10..24 #299
# NSIM       = 66                    # closed-loop steps
NMPC       = 5                      # prediction horizon
...
def closed_loop_with_noise(ctrls, F, user_input, sol, Nsim, outdir):
    ...
    for name, ctrl in ctrls.items():
        ...
        for k in range(Nsim):
            ...
            print(f"x_true : {x_true[x_pos_index]}, {x_true[y_pos_index]}, {x_true[z_pos_index]}")
            print(f"x_meas : {x_meas[x_pos_index]}, {x_meas[y_pos_index]}, {x_meas[z_pos_index]}")
            print(f"x_next : {x_next[x_pos_index]}, {x_next[y_pos_index]}, {x_next[z_pos_index]}")
...
def main():
    ...
    for ridx in range(lo, hi + 1):
        ...
        outdir = OUT_ROOT / f"T_{t}_N_{n}_beta_{beta}_accreg_{accrg}_NMPC_{10}_config{CONFIG}_WITH_DISTURBANCE_with_step"
        ensure_dir(outdir)
        # Run closed loop with disturbances
        log, log_ref = closed_loop_with_noise(
            ctrls, F, user_input, sol,
            Nsim=10, outdir=outdir
            # Nsim=NSIM
