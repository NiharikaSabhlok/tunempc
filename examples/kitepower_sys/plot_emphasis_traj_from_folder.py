...
acc_data=[1e8,1e7,8e7]
beta_data=[0.4,0.45,0.3,0.35,0.5]#,0.2,0.3,0.4,0.45,0.5]
...
for name in os.listdir(DATA_FOLDER/"plot_traj"):
    ...
    #         acc=float(f"{acc:.1f}")
        ...
        if acc in acc_data and beta in beta_data and t in[T_opti]:
...
colors=["#B21010","#1D0798","#1C9F0A","#9A0C68","#1E9CB6","#CF74C1","#173812F9","#1C9F0A","#1D0715","#CBC026"]
# --- 3D figure ---
...
for i, param in enumerate(final_param_list):
    ...
    if param not in grouped_data:
      ...
      # --- is this (beta, acc) one of the "good" ones? ---
    is_good =(beta, acc) in GOOD_COMBOS
    ...
    for entry in grouped_data[param]:
        *data_columns, _label_text = entry
        ...
        if is_good:
            label = rf"$\beta={beta}, \ \mathrm{{w}}_{{\ddot q}}={acc/1e8}$" if first_label else None
            ax.plot(x, y, z, color=colors[index], label=label, linewidth=1.4)
            draw_tethers_3d(ax, x, y, z, step=5, color_line=colors[index])
            index +=1
        else:
            ax.plot(
                x, y, z,
