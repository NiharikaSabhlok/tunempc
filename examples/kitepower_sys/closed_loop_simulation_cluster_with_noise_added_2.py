...
ROW_RANGE  = (342, 342)              # inclusive (0-based): process rows 10..24 #299
...
b_add_noise = True
CONFIG=1
indices_for_adding_noise=[35,36,37,38,39,40]
noise_to_be_added=[0.1,0.2,0.3,0.5,0.3,0.3]
DISTURB_VELOCITY = True
DISTURB_POSITION = False
x_pos_index=0
y_pos_index=1
z_pos_index=2
x_vel_index=3
...
def main():
    ...
    for ridx in range(lo, hi + 1):
        ...
        outdir = OUT_ROOT / f"T_{t}_N_{n}_beta_{beta}_accreg_{accrg}_NMPC_{NSIM}_with_logs_and_noise_config{CONFIG}"
        ensure_dir(outdir)
