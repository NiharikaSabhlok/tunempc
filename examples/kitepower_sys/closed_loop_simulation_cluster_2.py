...
ROW_RANGE  = (299, 299)              # inclusive (0-based): process rows 10..24
...
def save_all_logs_as_csv(outdir: Path, log, log_ref, sol, user_input):
    ...
    for nm in names:
        # x (Nsim+1, nx)
        # X = _stack_dm_rows(log["x"][nm])
        X = log["x"][nm]
        pd.DataFrame(np.column_stack([tx, X]),
                     columns=["t_cycle"] + [f"x{i}" for i in range(nx)]
                     ).to_csv(outdir/f"{nm}_x.csv", index=False)

        # u (Nsim, nu)
        # U = _stack_dm_rows(log["u"][nm])
        U = log["u"][nm]
        pd.DataFrame(np.column_stack([t, U]),
                     columns=["t_cycle"] + [f"u{i}" for i in range(nu)]
                     ).to_csv(outdir/f"{nm}_u.csv", index=False)

        # l, h, power (Nsim, 1)
        L = _to_float_array(log["l"][nm]).ravel()
        H = _to_float_array(log["h"][nm]).ravel()
        P = _to_float_array(log["power"][nm]).ravel()
        pd.DataFrame({"t_cycle": t, "l": L, "h": H, "power": P}
                     ).to_csv(outdir/f"{nm}_lhp.csv", index=False)

        # lambda: may be scalar or vector per step; pad to max length
        lam_list = log.get("lambda", {}).get(nm, [])
        if lam_list:
            # ensure 1D arrays
            lam_list = [np.atleast_1d(np.array(v).astype(float).ravel()) for v in lam_list]
            ...
            pd.DataFrame(np.column_stack([t, LAM]), columns=cols
...
def plot_and_save(outdir: Path, log, sol, user_input, state_idx=0, state_idx_2=2):
    ...
    for nm in names:
        ...
        plt.plot(tx, np.array(xs)-np.array(xref), label=nm, color=controller_colors[nm], linestyle=controller_linestyle[nm],linewidth=controller_linewidth[nm])
    plt.plot(tx, np.zeros_like(tx), "k--", linewidth=1)
    plt.grid(True); plt.legend(); plt.title(f"State deviation x[{state_idx_2}] - x_ref"); plt.xlabel("time [cycles]")
...
def plot_power_tracking(outdir: Path, log, log_ref, user_input):
    ...
    if "power_ref" in log_ref and len(log_ref["power_ref"]) >= Nsim:
        Pref = np.array(log_ref["power_ref"]).ravel()[:Nsim]
        plt.step(t, Pref, where="post", label="Reference", color=COLOR_OF_REFERENCE, linestyle=LINESTYLE_OF_REFERENCE, linewidth=LINEWIDTH_OF_REFERENCE)

    # controllers
    for nm in names:
        Pk = np.array([float(v) for v in log["power"][nm]]).ravel()
        plt.step(t, Pk, where="post", label=nm, alpha=0.9,color=controller_colors[nm], linestyle=controller_linestyle[nm],linewidth=controller_linewidth[nm])
...
def main():
    ...
    for ridx in range(lo, hi + 1):
        ...
        outdir = OUT_ROOT / f"T_{t}_N_{n}_beta_{beta}_accreg_{accrg}_NMPC_{NSIM}_with_logs"
        ensure_dir(outdir)
